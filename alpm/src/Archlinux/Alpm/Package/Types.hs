...
import Control.Exception (Exception, try) -- (Exception, displayException)
...
--
-- import Parsing.Decimal
-- import Parsing.DispositionTable
-- import Parsing.Error (CheckError (..), isCCharAt)
-- import Parsing.Error qualified as Parser
...
newtype AlpmPkgName = AlpmPkgName {unAlpmPkgName :: ShortText}
    deriving stock (Eq, Lift, Ord, Show)
    deriving (Display) via ShortText

-- instance IsString AlpmPkgName where
--     fromString = either (error . displayException) id . parsePkgNameFromString
...
name :: QuasiQuoter
name = quoter packageNameP

-- parseAlpmPkgName :: MonadThrow m => Text -> m AlpmPkgName
-- parseAlpmPkgName = either (throwM .AlpmPkgNameParseException) pure .
--     checkText AlpmPkgName pkgNameDispositionTable
--
-- parsePkgNameFromString :: String -> Either AlpmPkgNameParseException AlpmPkgName
-- parsePkgNameFromString = first AlpmPkgNameParseException .
--     checkString AlpmPkgName pkgNameDispositionTable
--
-- parsePkgNameFromCString :: CString -> IO (Either AlpmPkgNameParseException AlpmPkgName)
-- parsePkgNameFromCString = fmap (first AlpmPkgNameParseException) .
--     checkCString AlpmPkgName pkgNameDispositionTable

parsePackageNameFromText :: Text -> Either AlpmPkgNameParseException AlpmPkgName
parsePackageNameFromText =
    unsafeDupablePerformIO
        . flip Data.Text.Foreign.withCString parsePackageNameFromCString

parsePackageNameFromCString
    :: CString -> IO (Either AlpmPkgNameParseException AlpmPkgName)
parsePackageNameFromCString =
    fmap (first AlpmPkgNameParseException) . try . runParser packageNameP

packageNameP :: Parser AlpmPkgName
packageNameP = do
    start <- save
    check
        "valid first character"
        (isValidFirstCharacter packageNameDispositionTable)
    advance
    advanceWhile (isValidCharacter packageNameDispositionTable)
    isEndOfInput >>= \case
        True -> do
            string <- getSubstring start
            AlpmPkgName <$> liftIO (ShortText.unsafePackCStringLen string)
        False -> expected "valid character"

packageNameDispositionTable :: DispositionTable
packageNameDispositionTable =
    $( makeDispositionTable $ \c ->
        if
            | Data.Char.isAlphaNum c
                || c == '@'
                || c == '_'
                || c == '+' ->
                ValidFirstCharacter
            | c == '-'
                || c == '.' ->
                ValidCharacter
            | otherwise -> IllegalCharacter
     )

data AlpmPkgNameParseException = AlpmPkgNameParseException ParserError
    -- deriving (Eq, Show)
    deriving (Show)
...
version :: QuasiQuoter
version = quoter versionP

parseVersionFromText :: Text -> Either AlpmVersionParseException AlpmVersion
parseVersionFromText =
    unsafeDupablePerformIO
        . flip Data.Text.Foreign.withCString parseVersionFromCString

parseVersionFromCString
    :: CString -> IO (Either AlpmVersionParseException AlpmVersion)
parseVersionFromCString =
    fmap (first AlpmVersionParseException) . try . runParser versionP

versionP :: Parser AlpmVersion
versionP = do
    versionStart <- save
    ...
    version <-
        satisfies (isValidFirstCharacter versionDispositionTable) >>= \case
            True -> do
                advance
                advanceWhile (isValidCharacter versionDispositionTable)
                string <- getSubstring versionStart
                liftIO (ShortText.unsafePackCStringLen string)
            False -> expected "version"

    assert "hyphen" [cchar|-|]
    advance
    rel <-
        number >>= \case
            Nothing -> expected "rel"
            Just rel -> pure rel
    ...
    subrel <-
        is [cchar|.|] >>= \case
            True -> do
                advance
                number >>= \case
                    Nothing -> expected "subrel"
                    Just subrel -> pure (Just subrel)
            False -> pure Nothing
    ...
    pure
        AlpmVersion
            ...
            , alpmVersionVer = version
            , alpmVersionRel = rel
            , alpmVersionSubRel = subrel
            }

versionDispositionTable :: DispositionTable
versionDispositionTable =
    $( makeDispositionTable $ \c ->
        if Data.Char.isPrint c
            && (c /= ':') -- Technically this is not required by PKGBUILD(5).
            && (c /= '/')
            && (c /= '-')
            && (c /= ' ')
            then ValidFirstCharacter
...
showAlpmVersion x =
    let
        ...
        ver = ShortText.toString (alpmVersionVer x)
...
data AlpmVersionParseException = AlpmVersionParseException ParserError
    deriving (Eq, Show)

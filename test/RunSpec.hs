...
spec = do
    describe "dhalpm" $ do
        runWith "Empty" [relfile|empty.dhall|] $ withFs
            ( [ [reldir|db/|]
              , [reldir|db/local/|]
              , [reldir|root/|]
              ]
        ...
        describe "Install" $ do
            runWith "From syncdb" [relfile|install-from-syncdb.dhall|] $ withFs
                ( [ [reldir|db/|]
                  , [reldir|db/local/|]
            ...
            runWith "From file" [relfile|install-from-file.dhall|] $ withFs
                ( [ [reldir|db/|]
                  , [reldir|db/local/|]
                  , [reldir|db/local/test-package-1-1/|]
                  , [reldir|root/|]
            ...
            runWith "From build" [relfile|install-from-build.dhall|] $ withFs
                ( [ [reldir|db/|]
                  , [reldir|db/local/|]
                  , [reldir|db/local/test-package-1-1/|]
                  , [reldir|root/|]
            ...
            runWith "With dependency" [relfile|install-with-dependency.dhall|] $ withFs
                ( [ [reldir|db/|]
                  , [reldir|db/local/|]
            ...
            runWith "Providers" [relfile|providers.dhall|] $ withFs
                ( [ [reldir|db/|]
                  , [reldir|db/local/|]
            ...
            runWith' "Remove orphaned" [relfile|remove-orphaned.dhall|] (copyDatabase [reldir|localdb|]) $ withFs
                ( [ [reldir|db/|]
                  , [reldir|db/local/|]
                  --, [reldir|db/local/depends-package-1-1/|]
                  , [reldir|db/local/test-package-1-1/|]
...
runWith' :: String -> Path Rel File -> (Path Rel Dir -> IO ()) -> (Path Rel Dir -> Expectation) -> Spec
runWith' n config customSetup k = before_ setup $ it n $ do
    ...
    let lo' = setLogMinLevel (optionsLogLevel options) lo
    pc <- mkDefaultProcessContext
    withLogFunc lo' $ \lf -> let
        app = App
            { appLogFunc        = lf

...
run = do
    ...
    let
        opts = set substitutions subst' defaultEvaluateSettings
...
run'
    :: Path Abs Dir -> Path Abs Dir -> Vector Package -> AlpmHandlePtr -> RIO App ()
run' rootdir dbdir packages h = do
    ...
    dbs <- case collectDatabases packages of
        ...
        Success dbs -> do
            ...
            pure dbs'
    withTrans h [AlpmTransFlagAlldeps, AlpmTransFlagNeeded] $ do
...
getSyncPkg h pkg@(Package{..}) = do
    ...
    let
        ns = V.map databaseNameToString databases
    dbs <-
        getSyncdbs h
            >>= filterM
                ( \db -> do
...
registerDatabaseGlobal h Database{..} = do
    ...
    db <-
        registerSyncdb h (T.unpack name) $ toAlpmDatabaseSiglevels sigcheck sigtrust
